...
// Implementacion del framebuffer de MOFONGO32
// Mantiene doble buffer y helpers de primitivas para que GameApi/BackgroundApi no toquen pixels a mano
bool Graphics::s_atariColorMode = false;
bool Graphics::s_legacyGrayMapping = true;
int Graphics::s_drawOffsetX = 0;
int Graphics::s_drawOffsetY = 0;
unsigned char Graphics::s_globalHue = 0;
...
void Graphics::setColorModeAtari(bool enabled)
{
  s_atariColorMode = enabled;
}

void Graphics::setDrawOffset(int x, int y)
{
  s_drawOffsetX = x;
  s_drawOffsetY = y;
}

int Graphics::drawOffsetX()
{
  return s_drawOffsetX;
}

int Graphics::drawOffsetY()
{
  return s_drawOffsetY;
}

void Graphics::setGlobalHue(unsigned char hue)
{
  s_globalHue = hue & 0x0F;
}

void Graphics::setLegacyGrayMapping(bool enabled)
{
  s_legacyGrayMapping = enabled;
}

unsigned char Graphics::globalHue()
{
  return s_globalHue;
}
...
void Graphics::init()
{
  ...
  for(int y = 0; y < yres; y++)
  {
    ...
    memset(frame[y], 0, xres);
    memset(backbuffer[y], 0, xres);
  ...
  if(clear > -1)
    for(int y = 0; y < yres; y++)
      for(int x = 0; x < xres; x++)
        backbuffer[y][x] = encodeColor((char)clear);
  triangleCount = 0;
  triangleRoot = 0;
}
...
void Graphics::enqueueTriangle(short *v0, short *v1, short *v2, char color)
{
...
void Graphics::triangle(short *v0, short *v1, short *v2, char color)
{
  short *v[3] = {v0, v1, v2};
  if(v[1][1] < v[0][1])
  {
...
void Graphics::rect(int x, int y, int w, int h, int color)
{
...
void Graphics::circle(int cx, int cy, int r, int color)
{
  if(r <= 0)
    return;
  int x = r;
...
void Graphics::fillCircle(int cx, int cy, int r, int color)
{
  if(r <= 0)
    return;
  int x = r;
  ...
  while(x >= y)
  {
    xLine(cx - x, cx + x + 1, cy + y, color);
    xLine(cx - x, cx + x + 1, cy - y, color);
    xLine(cx - y, cx + y + 1, cy + x, color);
    xLine(cx - y, cx + y + 1, cy - x, color);
    y++;
    if(err < 0)
    {
