...
class AudioSystem
{
  ...
  void calcSamples()
  {
    int filled = writePosition - readPosition;
    if(filled < 0)
      filled += bufferSize;

    int samples = targetFill - filled;
    if(samples <= 0)
      return;

    int freeSpace = (bufferSize - 1) - filled;
  ...
  {
    //If producer is late, output silence instead of replaying stale samples.
