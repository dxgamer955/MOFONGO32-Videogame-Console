...
{
// Este archivo es el interprete principal de MOFONGO32
// Flujo por frame:
// 1) lee input/tiempo
// 2) ejecuta opcodes MASA y acciones por senal
// 3) actualiza fisica simple/collisions/alarms
// 4) llena CommandQueue para que el sketch dibuje o envie por SPI
...
{
  // MASA guarda floats little-endian igual que ESP32; memcpy evita aliasing raro
  float v = 0.0f;
  memcpy(&v, p, sizeof(float));
  return v;
}

// Pools por objeto. Se usan arrays paralelos para ahorrar RAM/overhead y mantener acceso rapido
...
// Overlays persistentes de texto/shape que el bytecode puede prender/apagar
...
// UI simple para dialogos y choices. Pequeno por diseno: el ESP32 no perdona RAM sobrante
...
static bool s_choiceConfirmConsumed;
// Hitboxes y senales: collision detecta eventos, signal actions deciden que pasa despues
...
// Acciones por senal van en heap para quitar presion del .bss en builds con Bluepad32
...
// Estado global de cuarto: scroll, alarms, musica, HUD y variables tipo GML
...
{
  // Velocidad random en escala x10; usado para asteroides/proyectiles sin floats en bytecode
...
{
  // Reemplaza placeholders del HUD como {score}; mantiene el buffer dentro del limite fijo
...
{
  // Lazy allocation: solo se reserva si el runtime MASA se usa en este build
...
{
  // AABB centrado en la posicion del objeto
...
static bool hitbox_overlap_swept(float ax0, float ay0, float ax1, float ay1, uint8_t aw, uint8_t ah,
                                 float bx, float by, uint8_t bw, uint8_t bh)
{
  // Barrido barato: evita que balas rapidas atraviesen objetos entre frames
  float halfAw = aw * 0.5f;
  float halfAh = ah * 0.5f;
  float halfBw = bw * 0.5f;
  float halfBh = bh * 0.5f;
...
{
  // Marca objetos persistentes mirando todas las rooms antes de resetear el room actual
...
{
  // Reset fuerte del runtime. No libera pools; solo limpia estado para iniciar juego/room
...
void Runtime::step(uint32_t nowMs, CommandQueue &out)
{
  ...
  if(m_persistent)
  {
    ...
    bool confirmNowGlobal = (s_inputMask & (INPUT_A | INPUT_START)) != 0;
    if(!confirmNowGlobal)
      s_choiceConfirmConsumed = false;
    ...
    for(int i = 0; i < kMaxChoiceSlots; i++)
    {
      ...
      bool confirmNow = confirmNowGlobal;
      ...
      if(confirmNow && !s_choiceConfirmConsumed)
      {
